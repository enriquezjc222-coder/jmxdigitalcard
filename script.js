...
function initTheme(){const panel=$id("themePanel"),options=panel?.querySelector(".theme-options");if(options&&planAllows("profileThemes")){options.innerHTML=Object.entries(themes).map(([id,a])=>`<button class="theme-option" data-theme="${id}"><span class="theme-circle" style="background:linear-gradient(135deg,${a[2]},${a[0]},${a[1]})"></span>${id.replaceAll("_"," ").replace(/\b\w/g,c=>c.toUpperCase())}</button>`).join("")}else if(!planAllows("profileThemes")){$id("themeButton")?.remove();panel?.remove()}$id("themeButton")?.addEventListener("click",()=>panel?.classList.add("active"));$id("closeThemePanel")?.addEventListener("click",()=>panel?.classList.remove("active"));panel?.addEventListener("click",e=>{const b=e.target.closest(".theme-option");if(!b)return;applyTheme(b.dataset.theme);panel.classList.remove("active")})}
