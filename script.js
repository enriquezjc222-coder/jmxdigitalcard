...
async function submitQuickCapture(event){
  ...
  if(effectivePublicPlan()!=="Business"||!planAllows("quickCapture")||!planAllows("leads")){if(status)status.textContent="Quick Capture is not available for this card.";return}
...
function validQrColors(){const dark=/^#[0-9a-f]{6}$/i.test(p.qrDarkColor||"")?p.qrDarkColor:"#111111",light=/^#[0-9a-f]{6}$/i.test(p.qrLightColor||"")?p.qrLightColor:"#ffffff";const lum=h=>{const n=parseInt(h.slice(1),16),r=(n>>16)&255,g=(n>>8)&255,b=n&255;return .2126*r+.7152*g+.0722*b};return Math.abs(lum(dark)-lum(light))>=110?[dark,light]:["#111111","#ffffff"]}
