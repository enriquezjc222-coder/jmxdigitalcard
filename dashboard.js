...
(function initJmxAdminTooltips(){
  ...
  function show(el,force=false){if(!force&&!finePointer())return;const text=helpFor(el);if(!text)return;current=el;clearTimeout(timer);timer=setTimeout(()=>{if(current!==el)return;const t=ensure();t.textContent=text;t.classList.add("is-visible");requestAnimationFrame(()=>place(el));el.setAttribute("aria-describedby","jmxAdminTooltip")},300)}
