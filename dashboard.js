...
// Keep legacy/public URL cards and official NFC-ID records in separate dashboard lanes.
// Existing legacy records without recordType remain Public URL records unless their
// established creation metadata proves they came from the official NFC-ID flow.
function isNfcIdCard(card = {}) {
  const explicit = String(card.recordType || card.inventory?.recordType || card.admin?.recordType || "").toLowerCase();
  if (explicit === "nfc_id") return true;
  if (explicit === "public_url") return false;
  const creation = String(card.creationMethod || card.admin?.creationMethod || "").toLowerCase();
  const physical = String(card.inventory?.physicalType || card.admin?.physicalType || "").toLowerCase();
  return creation.startsWith("official-device-id") || physical.includes("device-id");
}
function isPublicUrlCard(card = {}) { return !isNfcIdCard(card); }
...
function renderMasterSearchResults(query) {
  ...
  const matches = cards.filter(isPublicUrlCard).filter(card => cardSearchText(card).includes(query) && (statusFilter === "all" || card.status === statusFilter) && (planFilter === "all" || effectivePlan(card) === planFilter));
  box.hidden = false;
...
function render() {
  ...
  const planFilter = $("planFilter")?.value || "all";
  const publicCards = cards.filter(isPublicUrlCard);
  const list = publicCards.filter((card) => {
  ...
  // These counters belong to the legacy/Public URL inventory only. NFC-ID records
  // are counted independently in the NFC Cards & Batches section below.
  $("totalCards").textContent = publicCards.length;
  $("availableCards").textContent = publicCards.filter((c) => c.status === "available").length;
  $("activatedCards").textContent = publicCards.filter((c) => c.status === "activated").length;
  $("suspendedCards").textContent = publicCards.filter((c) => c.status === "suspended").length;
  const clientStatuses = new Set(["activated", "suspended"]);
  const unclaimedStatuses = new Set(["available", "sold"]);
  $("basicCards").textContent = publicCards.filter(c=>clientStatuses.has(c.status) && effectivePlan(c)==="Basic").length;
  $("premiumCards").textContent = publicCards.filter(c=>clientStatuses.has(c.status) && effectivePlan(c)==="Premium").length;
  $("businessCards").textContent = publicCards.filter(businessCountsAsActive).length;
  $("compCards").textContent = publicCards.filter(c=>c.complimentaryPremium===true).length;
  $("compBusinessCards").textContent = publicCards.filter(c=>c.complimentaryBusiness===true).length;
  $("businessAvailableCards").textContent = publicCards.filter(c=>effectivePlan(c)==="Business" && c.status==="available").length;
  $("businessSoldCards").textContent = publicCards.filter(c=>effectivePlan(c)==="Business" && c.status==="sold").length;
  $("inactiveCards").textContent = publicCards.filter(c=>unclaimedStatuses.has(c.status) || !c.status).length;
  ...
  $("clientDetailBody").innerHTML = `
    <div class="client-detail-grid">
      <section class="detail-panel">
        ...
        <div class="detail-list">
          <div><span>Card code</span><strong>${esc(id)}</strong></div>
          <div><span>Account ID</span><strong>${esc(card.accountId || "Pending identity sync")}</strong></div>
          <div><span>Identity Profile ID</span><strong>${esc(card.identityProfileId || "Pending identity sync")}</strong></div>
...
async function setComplimentaryTier(id, card, tier, enabled){
  ...
  await loadCards();renderNfcCenter();
}
...
async function createCard(event) {
  ...
  try {
    ...
    const batch = writeBatch(db);
    batch.set(doc(db, "cards", id), { inventoryVersion: 2, recordType: "public_url", status: "available", plan, complimentaryPremium:false, complimentaryBusiness:false, subscription:{status:"none",source:"manual"}, nfcStatus, requiresActivationCode, createdAt: serverTimestamp(), updatedAt: serverTimestamp() });
    batch.set(doc(db, "inventory", id), { activationCode: code, inventoryVersion: 2, recordType: "public_url", status: "available", plan, physicalType, nfcStatus, notes, requiresActivationCode, createdAt: serverTimestamp(), updatedAt: serverTimestamp() });
    batch.set(doc(db, "cardAdmin", id), { recordType: "public_url", physicalType, nfcStatus, notes, createdAt: serverTimestamp(), updatedAt: serverTimestamp() });
...
function closeInventoryPanel(){
  ...
  const button=expandedInventoryPanel.querySelector("[data-panel-expand]");
  button?.setAttribute("aria-expanded","false");
  if(button){button.setAttribute("aria-label",button.dataset.expandLabel||"Expand list");button.title="Expand";}
...
function openInventoryPanel(key){
  ...
  closeInventoryPanel(); expandedInventoryPanel=panel; panel.classList.add("inventory-panel-expanded"); panel.querySelector(".compact-record-list")?.classList.add("is-expanded");
  const button=panel.querySelector("[data-panel-expand]");
  if(button){button.dataset.expandLabel||=button.getAttribute("aria-label")||"Expand list";button.setAttribute("aria-expanded","true");button.setAttribute("aria-label","Close expanded list");button.title="Close";}
...
// Prueba 5: close client details safely from the backdrop or Escape without click-through.
$("clientDetailDialog")?.addEventListener("click",(event)=>{if(event.target===$("clientDetailDialog")){event.preventDefault();event.stopPropagation();closeClientDetailSafely();}});
$("clientDetailDialog")?.addEventListener("cancel",(event)=>{event.preventDefault();closeClientDetailSafely();});
...
onAuthStateChanged(auth, async (currentUser) => {
  ...
  try {
    ...
    await Promise.all([loadCards(),loadPlatformSettings(),loadAiScannerAdmin(),loadActivityLog()]);
  } catch (error) {
    console.error(error);
...
const NFC_TYPE_LABELS={
  card:"NFC Card (Legacy)","pvc-card":"PVC NFC Card","metal-card":"Metal NFC Card","wood-card":"Wood NFC Card","paper-card":"Paper/Cardboard NFC Card",
  sticker:"NFC Sticker",tag:"NFC Tag",keychain:"NFC Keychain / Key Fob",wristband:"NFC Wristband",bracelet:"NFC Bracelet",ring:"NFC Ring",plate:"NFC Plate",badge:"NFC Badge","id-badge":"NFC ID Badge","name-tag":"NFC Name Tag",token:"NFC Token",coin:"NFC Coin",disc:"NFC Disc",button:"NFC Button",label:"NFC Label",inlay:"NFC Inlay","wet-inlay":"NFC Wet Inlay","dry-inlay":"NFC Dry Inlay","epoxy-tag":"NFC Epoxy Tag","silicone-tag":"NFC Silicone Tag","laundry-tag":"NFC Laundry Tag","cable-tie-tag":"NFC Cable Tie Tag","nail-tag":"NFC Nail Tag","screw-tag":"NFC Screw Tag","anti-metal-tag":"NFC Anti-Metal Tag","on-metal-tag":"NFC On-Metal Tag","flexible-tag":"NFC Flexible Tag","smart-poster":"NFC Smart Poster","display-sign":"NFC Display / Sign","table-tent":"NFC Table Tent",stand:"NFC Stand","business-card":"NFC Business Card","access-card":"NFC Access Card","hotel-key-card":"NFC Hotel Key Card","membership-card":"NFC Membership Card","event-badge":"NFC Event Badge","luggage-tag":"NFC Luggage Tag","pet-tag":"NFC Pet Tag","phone-tag":"NFC Phone Tag",wearable:"NFC Wearable",other:"Other NFC Device",custom:"Custom NFC Device"
};
const NFC_SETTING_TYPES=Object.keys(NFC_TYPE_LABELS).filter(t=>t!=="card");
const NFC_QUICK_TYPES=Object.fromEntries(NFC_SETTING_TYPES.map(deviceType=>[deviceType,{deviceType,material:null}]));
NFC_QUICK_TYPES["pvc-card"]={deviceType:"pvc-card",material:"PVC"};NFC_QUICK_TYPES["metal-card"]={deviceType:"metal-card",material:"Metal"};
function nfcTypeLabel(type){return NFC_TYPE_LABELS[type]||type||"NFC Device";}
function isNfcCardType(type){return ["card","pvc-card","metal-card","wood-card","paper-card","business-card","access-card","hotel-key-card","membership-card"].includes(type||"card");}
...
function nfcDeviceBaseUrl(id){return `https://jmxdigitalcard.com/d/${encodeURIComponent(id)}`}
function nfcDeviceUrl(id,token=""){const base=nfcDeviceBaseUrl(id);return token?`${base}?t=${encodeURIComponent(token)}`:base}
function cachedNfcDeviceUrl(id){
  const cached=nfcSecureUrlCache.get(id);if(cached)return cached;
  const d=nfcDevices.find(x=>x.deviceId===id);
  return d?.secureTokenRequired===true||d?.tokenHash?"SECURE_TOKEN_PROTECTED__ROTATE_OR_REISSUE_TO_REENCODE":nfcDeviceBaseUrl(id);
}
async function provisionNfcTokens(deviceIds,{rotateExisting=false}={}){
  const ids=[...new Set((deviceIds||[]).filter(Boolean))],results=[];
  for(let i=0;i<ids.length;i+=200){
    const r=(await issueNfcDeviceTokensCall({deviceIds:ids.slice(i,i+200),rotateExisting})).data;
    for(const item of (r?.results||[])){
      results.push(item);
      if(item?.ok&&item.rawToken)nfcSecureUrlCache.set(item.deviceId,nfcDeviceUrl(item.deviceId,item.rawToken));
    }
  }
  return results;
}
async function rotateAndCacheNfcToken(deviceId){
  const r=(await rotateNfcDeviceTokenCall({deviceId})).data;
  if(!r?.rawToken)throw new Error("Secure token was not returned.");
  const url=nfcDeviceUrl(deviceId,r.rawToken);nfcSecureUrlCache.set(deviceId,url);return url;
}
...
// Single source of truth for the five NFC-ID dashboard buckets.
// nfcDevices is itself an official ID collection, so legacy device records that
// predate recordType are safely treated as nfc_id without migrating them.
function getNfcInventoryCategory(record = {}) {
  const status = String(record.status || record.lifecycleStatus || "available").toLowerCase();
  if (["disabled","suspended","lost","stolen","replaced","retired","archived"].includes(status)) return "disabled";
  const card = record.cardId ? cards.find(c => c.id === record.cardId) : null;
  if (record.cardId && status === "active" && card && String(card.status || "").toLowerCase() === "activated") {
    const plan = effectivePlan(card);
    if (plan === "Premium") return "premium";
    if (plan === "Business") return "business";
    return "basic";
  }
  if (status === "sold" || (record.cardId && ["pending","available"].includes(status))) return "sold_pending_activation";
  if (!record.cardId && ["available","pending"].includes(status)) return "available";
  return "disabled";
}
...
function renderNfcPlanClients(){
  ...
  const bucketIds={
    Basic:new Set(nfcDevices.filter(d=>getNfcInventoryCategory(d)==="basic").map(d=>d.cardId).filter(Boolean)),
    Premium:new Set(nfcDevices.filter(d=>getNfcInventoryCategory(d)==="premium").map(d=>d.cardId).filter(Boolean)),
    Business:new Set(nfcDevices.filter(d=>getNfcInventoryCategory(d)==="business").map(d=>d.cardId).filter(Boolean))
  };
  const buckets={
    Basic:cards.filter(c=>bucketIds.Basic.has(c.id)),
    Premium:cards.filter(c=>bucketIds.Premium.has(c.id)),
    Business:cards.filter(c=>bucketIds.Business.has(c.id))
  };
  const configs=[
...
function renderNfcCenter(){
  ...
  const cardsOnly=nfcDevices;
  const available=cardsOnly.filter(x=>getNfcInventoryCategory(x)==="available"),sold=cardsOnly.filter(x=>getNfcInventoryCategory(x)==="sold_pending_activation");
  const active=cardsOnly.filter(x=>["basic","premium","business"].includes(getNfcInventoryCategory(x)));
  const disabled=cardsOnly.filter(x=>getNfcInventoryCategory(x)==="disabled");
  $("nfcDeviceTotal").textContent=cardsOnly.length;$("nfcDeviceAvailable").textContent=available.length;$("nfcDeviceActive").textContent=active.length;$("nfcDeviceDisabled").textContent=disabled.length;$("nfcBatchTotal").textContent=nfcBatches.length;
...
async function markNfcCardSold(deviceId){const d=nfcDevices.find(x=>x.deviceId===deviceId);if(!d)return;if(!confirm("Mark this NFC card as SOLD and waiting for customer activation?"))return;const wb=writeBatch(db),now=serverTimestamp();wb.set(doc(db,"nfcDevices",deviceId),{recordType:"nfc_id",status:"sold",lifecycleStatus:"sold",soldAt:now,updatedAt:now},{merge:true});wb.set(doc(db,"nfcDevicePublic",deviceId),{status:"sold",updatedAt:now},{merge:true});if(d.batchId)wb.set(doc(db,"nfcBatches",d.batchId),{status:"sold",soldAt:now,updatedAt:now},{merge:true});wb.set(doc(db,"nfcDeviceEvents",`${Date.now()}-${deviceId}`),{event:"NFC Device Marked Sold",deviceId,batchId:d.batchId||null,deviceType:d.deviceType||"card",adminUid:user?.uid||"",createdAt:serverTimestamp()});await wb.commit();await loadNfcData();loadActivityLog();}
function renderNfcGlobalSearch(){
  ...
  const results=[...cm.map(c=>({kind:"client",id:c.id,title:c.profile?.fullName||c.admin?.clientName||c.owner?.ownerEmail||c.id,sub:`${c.id} · ${effectivePlan(c)} · ${c.status||""}`})),...bm.map(b=>({kind:"batch",id:b.batchId,title:b.batchId,sub:`${nfcTypeLabel(b.deviceType)} · ${b.activationCode||""} · ${b.cardId||"Unassigned"}`})),...dm.map(d=>({kind:"device",id:d.deviceId,title:d.deviceId,sub:`${nfcTypeLabel(d.deviceType)} · ${d.batchId||""} · ${d.cardId||"Unassigned"}`}))].slice(0,20);
...
async function createNfcQuickDevice(e){
  e.preventDefault();
  const choice=NFC_QUICK_TYPES[$("nfcQuickDeviceType")?.value]||NFC_QUICK_TYPES["pvc-card"],plan=$("nfcQuickDevicePlan")?.value||"Basic",notes=$("nfcQuickDeviceNotes")?.value.trim()||"",statusEl=$("nfcQuickDeviceStatus"),button=$("createNfcQuickDeviceButton");
  ...
  try{
  {
      ...
      wb.set(doc(db,"nfcBatches",batchId),{batchId,recordType:"nfc_id",activationCode,activationMode:"individual-device",deviceType:choice.deviceType,material:choice.material,quantity:1,plan,cardId:null,status:"available",enabled:true,notes,activationCodeStatus:"unused",encodingStatus:"not-started",encodedCount:0,createdBy:user?.uid||"",createdAt:now,updatedAt:now});
      wb.set(doc(db,"nfcDevices",deviceId),{deviceId,batchId,recordType:"nfc_id",activationCodeRef:batchId,deviceType:choice.deviceType,material:choice.material,plan,deviceNumber:1,cardId:null,status:"available",lifecycleStatus:"available",enabled:true,deviceEnabled:true,secureTokenRequired:true,securityMode:"STANDARD_TOKEN",deviceSecurityType:"legacy",chipModel:"NTAG21X_OR_STANDARD_NFC",secureUid:null,secureKeyVersion:null,sunEnabled:false,lastVerifiedCounter:null,lastVerifiedAt:null,tapCount:0,encodingStatus:"not-encoded",notes,createdAt:now,updatedAt:now});
      wb.set(doc(db,"nfcDevicePublic",deviceId),{deviceId,batchId,recordType:"nfc_id",deviceType:choice.deviceType,material:choice.material,plan,status:"available",enabled:true,updatedAt:now});wb.set(doc(db,"nfcDeviceEvents",`${Date.now()}-${deviceId}`),{event:"NFC Device Created",deviceId,batchId,deviceType:choice.deviceType,adminUid:user?.uid||"",createdAt:now});await wb.commit();await provisionNfcTokens([deviceId]);await loadNfcData();loadActivityLog();
    }
    statusEl.innerHTML=`Created <strong>${esc(deviceId)}</strong> · Activation Code <strong>${esc(activationCode)}</strong>`;
    await copyText(cachedNfcDeviceUrl(deviceId));
...
async function createNfcBatch(e){
  e.preventDefault();const quantity=Math.max(1,Math.min(5000,Number($("nfcBatchQuantity").value||1))),deviceType=$("nfcBatchType")?.value||"pvc-card",batchId=$("nfcBatchId").value,activationCode=$("nfcBatchActivationCode").value,cardId=$("nfcBatchCardId")?.value||null,plan=$("nfcBatchPlan")?.value||"Basic",notes=$("nfcBatchNotes").value.trim(),statusEl=$("nfcBatchStatus");statusEl.textContent=`Creating ${quantity} devices…`;
  ...
  try{
    await setDoc(doc(db,"nfcBatches",batchId),{batchId,recordType:"nfc_id",activationCode,deviceType,quantity,plan,cardId,status:initialStatus,enabled:true,notes,activationCodeStatus:"unused",encodingStatus:"not-started",encodedCount:0,createdBy:user?.uid||"",createdAt:now,updatedAt:now});
    for(let start=0;start<deviceIds.length;start+=180){const wb=writeBatch(db);for(const [offset,deviceId] of deviceIds.slice(start,start+180).entries()){const deviceNumber=start+offset+1;wb.set(doc(db,"nfcDevices",deviceId),{deviceId,batchId,recordType:"nfc_id",deviceType,deviceNumber,plan,cardId,status:initialStatus,enabled:true,secureTokenRequired:true,securityMode:"STANDARD_TOKEN",deviceSecurityType:"legacy",chipModel:"NTAG21X_OR_STANDARD_NFC",secureUid:null,secureKeyVersion:null,sunEnabled:false,lastVerifiedCounter:null,lastVerifiedAt:null,tapCount:0,encodingStatus:"not-encoded",notes:"",createdAt:serverTimestamp(),updatedAt:serverTimestamp()});wb.set(doc(db,"nfcDevicePublic",deviceId),{deviceId,batchId,recordType:"nfc_id",deviceType,plan,cardId,status:initialStatus,enabled:true,updatedAt:serverTimestamp()})}await wb.commit()}
    await setDoc(doc(db,"nfcDeviceEvents",`${Date.now()}-${batchId}`),{event:"NFC Batch Created",batchId,deviceType,quantity,adminUid:user?.uid||"",createdAt:serverTimestamp()});await provisionNfcTokens(deviceIds);loadActivityLog();
    statusEl.textContent=`Created ${quantity} ${nfcTypeLabel(deviceType)}(s). Activation code: ${activationCode}`;await copyText(activationCode);setTimeout(()=>{$("nfcBatchDialog")?.close();openNfcBatchEncoder(batchId)},700);await loadNfcData();
  }catch(err){console.error(err);statusEl.textContent="Could not create the batch: "+(err?.message||"unknown error")}
}
function closeNfcExpanded(){if(!expandedNfcBox)return;expandedNfcBox.classList.remove("is-expanded");const button=expandedNfcBox.querySelector("[data-nfc-expand]");button?.setAttribute("aria-expanded","false");if(button){button.setAttribute("aria-label",button.dataset.expandLabel||"Expand NFC list");button.title="Expand";}nfcBackdrop?.remove();nfcBackdrop=null;expandedNfcBox=null;document.body.classList.remove("nfc-device-panel-open")}
function expandNfcBox(key){const box=document.querySelector(`[data-nfc-box="${key}"]`);if(!box)return;if(expandedNfcBox===box)return closeNfcExpanded();closeNfcExpanded();expandedNfcBox=box;box.classList.add("is-expanded");const button=box.querySelector("[data-nfc-expand]");if(button){button.dataset.expandLabel||=button.getAttribute("aria-label")||"Expand NFC list";button.setAttribute("aria-expanded","true");button.setAttribute("aria-label","Close expanded NFC list");button.title="Close";}nfcBackdrop=document.createElement("div");nfcBackdrop.className="nfc-device-backdrop";nfcBackdrop.onclick=closeNfcExpanded;document.body.appendChild(nfcBackdrop);document.body.classList.add("nfc-device-panel-open")}
...
async function replaceNfcDevice(deviceId){
  ...
  const clean={...d};delete clean.lifecycleStatus;delete clean.replacedBy;delete clean.deviceEnabled;delete clean.replaces;delete clean.tokenHash;delete clean.tokenCreatedAt;delete clean.tokenRotatedAt;delete clean.tokenRotatedBy;
  wb.set(doc(db,"nfcDevices",next),{...clean,deviceId:next,status,lifecycleStatus:status,enabled:true,deviceEnabled:true,secureTokenRequired:true,securityMode:"STANDARD_TOKEN",deviceSecurityType:clean.deviceSecurityType||"legacy",chipModel:clean.chipModel||"NTAG21X_OR_STANDARD_NFC",secureUid:null,secureKeyVersion:null,sunEnabled:false,lastVerifiedCounter:null,lastVerifiedAt:null,replaces:deviceId,tapCount:0,encodingStatus:"not-encoded",createdAt:serverTimestamp(),updatedAt:serverTimestamp()},{merge:false});
  ...
  wb.set(doc(db,"nfcDeviceEvents",`${Date.now()}-${next}`),{event:"Device Replaced",deviceId:next,previousDeviceId:deviceId,batchId:d.batchId||null,cardId:d.cardId||null,adminUid:user?.uid||"",createdAt:serverTimestamp()});await wb.commit();await provisionNfcTokens([next]);await loadNfcData();alert(`Replacement created: ${next}`)
}
...
function renderNfcEncoder(){
  ...
  $("nfcEncoderTitle").textContent="Program / Encode Batch";$("nfcEncoderSubtitle").textContent=`${batch.batchId} · ${nfcTypeLabel(batch.deviceType)} · ${batch.cardId||"Unassigned inventory"}`;
...
async function openNfcBatchEncoder(batchId){
  const items=nfcBatchDevices(batchId);if(!items.length)return alert("This batch contains no devices.");
  nfcEncoderBatchId=batchId;const firstPending=items.findIndex(d=>!nfcEncodingDone(d));nfcEncoderIndex=firstPending>=0?firstPending:0;
  try{
    const need=items.filter(d=>!d.tokenHash&&!nfcSecureUrlCache.has(d.deviceId)).map(d=>d.deviceId);
    if(need.length)await provisionNfcTokens(need);
  }catch(e){console.warn("Secure NFC token provisioning deferred",e?.message)}
  renderNfcEncoder();$("nfcEncoderDialog")?.showModal();
}
...
async function verifyCurrentNfcDevice(){
  const items=nfcBatchDevices(nfcEncoderBatchId),d=items[nfcEncoderIndex];if(!d)return;
  const expected=nfcSecureUrlCache.get(d.deviceId);
  if(!expected){$("nfcEncoderStatus").textContent="Secure token is protected and is not available in this browser session. Rotate/Get Secure URL before re-encoding or verifying.";return}
  const entered=prompt("Verify the secure URL written/read from this NFC device. Paste or type it here:",expected);if(entered===null)return;
  if(entered.trim()!==expected){$("nfcEncoderStatus").textContent="Verification failed: the secure URL does not match this authorized Device ID/token.";await updateNfcEncoding(d.deviceId,"error");return}
  await updateNfcEncoding(d.deviceId,"verified");$("nfcEncoderStatus").textContent="Verified: secure URL matches the authorized Device ID/token.";
}
function exportNfcBatchCsv(batchId){const batch=nfcBatches.find(b=>b.batchId===batchId),items=nfcBatchDevices(batchId);if(!batch||!items.length)return;const rows=[["Batch ID","Device Number","Device Type","Device ID","NFC Resolver URL","Device Status","Encoding Status","Profile/Card","Encoded At"],...items.map((d,i)=>[batchId,d.deviceNumber||i+1,NFC_TYPE_LABELS[d.deviceType]||d.deviceType,d.deviceId,(nfcSecureUrlCache.get(d.deviceId)||"SECURE_TOKEN_NOT_AVAILABLE_IN_THIS_SESSION"),d.status||"",nfcEncodingLabel(d),d.cardId||"",d.encodedAt?.toDate?.()?.toISOString?.()||d.encodedAt||""])];const csv=rows.map(r=>r.map(v=>`"${String(v??"").replaceAll('"','""')}"`).join(",")).join("\r\n");const blob=new Blob([csv],{type:"text/csv;charset=utf-8"}),url=URL.createObjectURL(blob),a=document.createElement("a");a.href=url;a.download=`${batchId}-nfc-production.csv`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000)}

async function deleteNfcBatch(batchId,{reopenCardId=null}={}){
  const batch=nfcBatches.find(x=>x.batchId===batchId);if(!batch)return alert("NFC batch not found. Reload and try again.");
  const assignedCardId=batch.cardId||null;
  const assignedCard=assignedCardId?cards.find(c=>c.id===assignedCardId):null;
  const label=assignedCard?.profile?.fullName||assignedCard?.admin?.clientName||assignedCardId||"unassigned inventory";
  const type=nfcTypeLabel(batch.deviceType),knownCount=nfcDevices.filter(d=>d.batchId===batchId).length;
  if(!confirm(`¿Eliminar el lote ${batchId} (${knownCount} ${type}) asignado a ${label}?

Esto elimina SOLO este lote y sus Device IDs NFC. NO elimina la tarjeta/perfil principal del cliente, su Card ID, URL, QR, Google Wallet ni otros lotes.`))return false;
  if(!confirm(`CONFIRMACIÓN FINAL: el lote ${batchId} y todos sus Device IDs dejarán de funcionar permanentemente. La tarjeta/perfil principal ${assignedCardId||"del cliente"} se conservará. ¿Continuar?`))return false;
  try{
    const snap=await getDocs(query(collection(db,"nfcDevices"),where("batchId","==",batchId)));
    const docs=snap.docs;
    for(let i=0;i<docs.length;i+=180){
      const wb=writeBatch(db);
      docs.slice(i,i+180).forEach(d=>{wb.delete(d.ref);wb.delete(doc(db,"nfcDevicePublic",d.id))});
      await wb.commit();
    }
    const wb=writeBatch(db);
    wb.delete(doc(db,"nfcBatches",batchId));
    wb.set(doc(db,"nfcDeviceEvents",`${Date.now()}-${batchId}-deleted`),{event:"NFC Batch Deleted",batchId,previousCardId:assignedCardId,deviceType:batch.deviceType||null,deviceCount:docs.length,adminUid:user?.uid||"",result:"Batch and Device IDs deleted; primary card/profile preserved",createdAt:serverTimestamp()});
    await wb.commit();
    await loadNfcData();loadActivityLog();
    $("nfcDeviceDialog")?.close();
    if(reopenCardId&&cards.some(c=>c.id===reopenCardId))await openClientDialog(reopenCardId,false);
    alert(`Lote ${batchId} eliminado. Se eliminaron ${docs.length} Device IDs NFC. La tarjeta/perfil principal y su Card ID NO fueron eliminados.`);
    return true;
  }catch(e){console.error(e);alert(e?.message||"No se pudo eliminar el lote NFC.");return false}
}

async function openNfcBatchDetails(batchId){const b=nfcBatches.find(x=>x.batchId===batchId);if(!b)return;const items=nfcDevices.filter(x=>x.batchId===batchId),card=cards.find(x=>x.id===b.cardId),dialog=$("nfcDeviceDialog");$("nfcDeviceDialogTitle").textContent=batchId;$("nfcDeviceDialogSubtitle").textContent=`${nfcTypeLabel(b.deviceType)} batch · ${items.length} devices`;$("nfcDeviceDialogBody").innerHTML=`<div class="client-detail-grid"><section class="detail-panel"><h3>Batch Information</h3><div class="detail-list"><div><span>Activation Code</span><strong class="nfc-copy-code">${esc(b.activationCode||"—")}</strong></div><div><span>Activation status</span><strong>${esc(b.activationCodeStatus||"unused")}</strong></div><div><span>Assigned profile</span><strong>${esc(card?.profile?.fullName||card?.admin?.clientName||b.cardId||"Unassigned")}</strong></div><div><span>Quantity</span><strong>${items.length}</strong></div></div><label class="form-group"><span>Assign / Transfer Batch</span><select id="nfcAssignBatchCard"><option value="">Choose client/profile</option>${cards.filter(c=>["activated","suspended"].includes(c.status)).map(c=>`<option value="${esc(c.id)}" ${c.id===b.cardId?"selected":""}>${esc(c.profile?.fullName||c.admin?.clientName||c.owner?.ownerEmail||c.id)} — ${esc(c.id)}</option>`).join("")}</select></label><div class="nfc-device-mini-list">${items.map(d=>`<div class="nfc-device-mini-row"><span><strong>${esc(d.deviceId)}</strong><small>${esc(d.status||"")} · ${Number(d.tapCount||0)} taps</small></span><span class="nfc-device-status-pill ${nfcStatusClass(d.status)}">${esc(d.status||"")}</span><button class="mini-button" type="button" data-open-device="${esc(d.deviceId)}">Open</button></div>`).join("")}</div></section></div>`;$("nfcDeviceDialogActions").innerHTML=`<button class="primary-button" data-program-batch>${items.some(d=>!nfcEncodingDone(d))?"Program / Continue Devices":"Review Encoded Devices"}</button><button class="secondary-button" data-export-batch>Export Batch CSV</button><button class="secondary-button" data-copy-batch-code>Copy Activation Code</button><button class="secondary-button" data-copy-activation-link>Copy Activation Page</button><button class="secondary-button" data-assign-batch>Assign / Transfer</button>${b.enabled===false||b.status==="disabled"?'<button class="primary-button" data-toggle-batch="on">Enable Batch</button>':'<button class="danger-button" data-toggle-batch="off">Disable Batch</button>'}<button class="danger-button" data-delete-nfc-batch>Eliminar lote</button>`;dialog.showModal();dialog.querySelector("[data-program-batch]")?.addEventListener("click",()=>{dialog.close();openNfcBatchEncoder(batchId)});dialog.querySelector("[data-export-batch]")?.addEventListener("click",()=>exportNfcBatchCsv(batchId));dialog.querySelectorAll("[data-open-device]").forEach(x=>x.onclick=()=>{dialog.close();openNfcDeviceDialog(x.dataset.openDevice)});dialog.querySelector("[data-copy-batch-code]")?.addEventListener("click",()=>copyText(b.activationCode||""));dialog.querySelector("[data-copy-activation-link]")?.addEventListener("click",()=>copyText(`${location.origin}/device-activate.html?code=${encodeURIComponent(b.activationCode||"")}`));dialog.querySelector("[data-assign-batch]")?.addEventListener("click",async()=>{const target=dialog.querySelector("#nfcAssignBatchCard")?.value;if(!target)return alert("Choose a client/profile first.");await assignNfcBatch(batchId,target);dialog.close()});dialog.querySelector('[data-toggle-batch="on"]')?.addEventListener("click",async()=>{await setNfcBatchEnabled(batchId,true);dialog.close()});dialog.querySelector('[data-toggle-batch="off"]')?.addEventListener("click",async()=>{await setNfcBatchEnabled(batchId,false);dialog.close()});dialog.querySelector("[data-delete-nfc-batch]")?.addEventListener("click",()=>deleteNfcBatch(batchId,{reopenCardId:b.cardId||null}))}
...
function clientNfcSection(card){
  const ds=nfcDevices.filter(d=>d.cardId===card.id);
  const bs=nfcBatches.filter(b=>b.cardId===card.id);
  const counts=Object.fromEntries(NFC_SETTING_TYPES.map(t=>[t,ds.filter(d=>d.deviceType===t).length]));
  if(ds.some(d=>d.deviceType==="card"))counts.card=ds.filter(d=>d.deviceType==="card").length;
  const usedTypes=[...new Set(ds.map(d=>d.deviceType||"card"))];
  const typeSummary=(usedTypes.length?usedTypes:["pvc-card","metal-card","sticker","keychain","wristband","ring","plate"]).map(type=>`<article><span>${esc(nfcTypeLabel(type))}</span><strong>${Number(counts[type]||0)}</strong></article>`).join("");
  const controls=(usedTypes.length?usedTypes:NFC_SETTING_TYPES).map(type=>{const on=card.nfcDeviceControls?.[type]!==false;return `<button type="button" class="nfc-control-button ${on?"":"off"}" data-client-nfc-type="${esc(type)}" data-client-nfc-state="${on?"on":"off"}"><span>${esc(nfcTypeLabel(type))}</span><strong>${on?"ON":"OFF"}</strong></button>`}).join("");
  return `<section class="detail-panel"><div class="nfc-client-section-title"><div><h3>Official JMX NFC Devices</h3><p class="subtitle small">All official NFC product types can point to this same digital profile. Existing URL cards, QR and Google Wallet remain compatible and separate.</p></div><span class="nfc-device-status-pill ${nfcStatusClass(profileLifecycle(card))}">${esc(profileLifecycle(card).toUpperCase())}</span></div>
  <div class="nfc-client-overview"><article><span>Plan</span><strong>${esc(effectivePlan(card)==="Business"?"JMX Business":effectivePlan(card))}</strong></article><article><span>Profile ID</span><strong>${esc(card.profileId||card.admin?.profileId||"Legacy profile")}</strong></article><article><span>Total NFC Devices</span><strong>${ds.length}</strong></article><article><span>Batches</span><strong>${bs.length}</strong></article></div>
  ...
  <div class="nfc-client-device-summary">${typeSummary}</div>
  ...
  <h4>NFC Product Controls — This Client Only</h4><div class="nfc-control-grid">${controls}</div>
  <h4>NFC Batches</h4><div class="nfc-device-mini-list">${bs.length?bs.map(b=>{const amount=nfcDevices.filter(d=>d.batchId===b.batchId).length;return `<div class="nfc-device-mini-row nfc-batch-client-row"><span><strong>${esc(b.batchId)}</strong><small>${amount} × ${esc(nfcTypeLabel(b.deviceType))} · ${esc(b.status||"available")}</small></span><span class="nfc-device-status-pill ${nfcStatusClass(b.status)}">${b.enabled===false?"OFF":"ON"}</span><button type="button" class="mini-button" data-client-open-batch="${esc(b.batchId)}">Open / Control</button><button type="button" class="mini-button nfc-delete-batch-button" data-client-delete-batch="${esc(b.batchId)}">Eliminar lote</button></div>`}).join(""):'<p class="subtitle small">No NFC batches assigned yet.</p>'}</div>
  <h4>Individual NFC Devices</h4><div class="nfc-device-mini-list">${ds.slice(0,80).map(d=>`<div class="nfc-device-mini-row"><span><strong>${esc(d.deviceId)}</strong><small>${esc(nfcTypeLabel(d.deviceType))} · ${esc(d.batchId||"")}</small></span><span class="nfc-device-status-pill ${nfcStatusClass(d.lifecycleStatus||d.status)}">${esc(d.lifecycleStatus||d.status||"")}</span><button type="button" class="mini-button" data-client-open-device="${esc(d.deviceId)}">Open</button></div>`).join("")||'<p class="subtitle small">No NFC devices assigned yet.</p>'}</div>
...
if((await getDoc(doc(db,"cards",cardId))).exists())throw new Error("Card ID already exists. Press Random and try again.");
    ...
    wb.set(doc(db,"cards",cardId),{inventoryVersion:2,recordType:"nfc_id",status:"activated",profileStatus:"active",profileId,creationMethod:"official-device-id-profile",plan,complimentaryPremium:false,complimentaryBusiness:false,subscription:{status:"none",source:"manual"},nfcStatus:"not-programmed",requiresActivationCode:false,createdAt:serverTimestamp(),updatedAt:serverTimestamp()});
    wb.set(doc(db,"profiles",cardId),profile);
    wb.set(doc(db,"inventory",cardId),{inventoryVersion:2,recordType:"nfc_id",status:"activated",plan,physicalType:"Profile / Device-ID",nfcStatus:"not-programmed",notes,requiresActivationCode:false,activationCode:"",createdAt:serverTimestamp(),updatedAt:serverTimestamp()});
    wb.set(doc(db,"cardAdmin",cardId),{recordType:"nfc_id",profileId,profileStatus:"active",creationMethod:"official-device-id-profile",clientName:fullName,physicalType:"Profile / Device-ID",nfcStatus:"not-programmed",notes,createdAt:serverTimestamp(),updatedAt:serverTimestamp()});
...
async function openNfcDeviceDialog(deviceId){
  ...
  $("nfcDeviceDialogTitle").textContent=card?.profile?.fullName||d.notes||batch?.notes||"NFC Device";$("nfcDeviceDialogSubtitle").textContent=`${nfcTypeLabel(d.deviceType)}${d.material?" · "+d.material:""} · ${life}`;
  $("nfcDeviceDialogBody").innerHTML=`<div class="client-detail-grid"><section class="detail-panel"><h3>NFC Device</h3><div class="detail-list"><div><span>Plan</span><strong>${esc(card?effectivePlan(card):(d.plan||batch?.plan||"Basic"))}</strong></div><div><span>Status</span><strong>${esc(life.toUpperCase())}</strong></div><div><span>Client</span><strong>${esc(card?.profile?.fullName||card?.admin?.clientName||"Unassigned — customer activates later")}</strong></div><div><span>Private Note</span><strong>${esc(d.notes||batch?.notes||"—")}</strong></div><div><span>Total taps</span><strong>${Number(d.tapCount||0).toLocaleString()}</strong></div></div><details class="nfc-advanced-details"><summary>Advanced Details</summary><div class="detail-list"><div><span>Internal Device ID</span><strong class="break-anywhere">${esc(deviceId)}</strong></div><div><span>Security Type</span><strong>${esc(String(d.deviceSecurityType||"legacy").toUpperCase())}</strong></div><div><span>Chip Model</span><strong>${esc(d.chipModel||"Legacy / Standard NFC")}</strong></div><div><span>Secure NFC Future Mode</span><strong>${d.deviceSecurityType==="secure"?"Configured — backend cryptographic verification required":"Legacy token authorization (current hardware)"}</strong></div><div><span>Resolver URL</span><strong class="break-anywhere">${esc(nfcSecureUrlCache.get(deviceId)||"Protected — open encoder and rotate/get secure URL to reprogram")}</strong></div><div><span>Activation Code</span><strong>${esc(batch?.activationCode||"—")}</strong></div><div><span>Batch</span><strong>${esc(d.batchId||"—")}</strong></div><div><span>Encoding</span><strong>${esc(d.encodingStatus||"not-encoded")}</strong></div></div></details><div id="nfcDeliveryKitQr" class="nfc-delivery-qr" hidden></div></section></div>`;
  ...
  dialog.showModal();
  dialog.querySelector("[data-nfc-copy-url]")?.addEventListener("click",async()=>{let url=nfcSecureUrlCache.get(deviceId);if(!url){if(!confirm("The raw secure token is not stored. Rotate the token and invalidate any previously encoded copy for this Device ID?"))return;url=await rotateAndCacheNfcToken(deviceId);await loadNfcData();}await copyText(url)});
...
const issueNfcDeviceTokensCall=httpsCallable(functions,"issueNfcDeviceTokens");
const rotateNfcDeviceTokenCall=httpsCallable(functions,"rotateNfcDeviceToken");
const setAccountAllowedDeviceCountCall=httpsCallable(functions,"setAccountAllowedDeviceCount");
const nfcSecureUrlCache=new Map();
...
// ===== Admin Activity Log =====
let adminActivity=[];
function activityCategory(x){const e=String(x?.event||"").toLowerCase();if(/batch/.test(e)||x?.batchId)return "batch";if(/device|encoding|nfc type/.test(e)||x?.deviceId)return "device";if(/profile|client|card/.test(e)||x?.cardId)return "profile";return "settings"}
function activityDate(v){try{if(v?.toDate)return v.toDate();if(v?.seconds)return new Date(v.seconds*1000);if(v)return new Date(v)}catch{}return null}
function renderActivityLog(){const host=$("activityLogList"),empty=$("activityLogEmpty");if(!host)return;const q=($("activitySearch")?.value||"").trim().toLowerCase(),type=$("activityTypeFilter")?.value||"all";const rows=adminActivity.filter(x=>(type==="all"||activityCategory(x)===type)&&(!q||[x.event,x.cardId,x.deviceId,x.batchId,x.adminUid,x.result].filter(Boolean).join(" ").toLowerCase().includes(q))).slice(0,150);host.innerHTML=rows.map(x=>{const dt=activityDate(x.createdAt),when=dt&&!Number.isNaN(dt.valueOf())?dt.toLocaleString():"Pending timestamp";return `<article class="activity-log-row"><div><strong>${esc(x.event||"Administrative action")}</strong><small>${esc([x.cardId&&`Card ${x.cardId}`,x.deviceId&&`Device ${x.deviceId}`,x.batchId&&`Batch ${x.batchId}`].filter(Boolean).join(" · ")||"Platform")}</small></div><div class="activity-log-meta"><span>${esc(activityCategory(x))}</span><time>${esc(when)}</time></div></article>`}).join("");if(empty)empty.hidden=rows.length>0}
async function loadActivityLog(){if(!$("activityLogList"))return;try{const snap=await getDocs(collection(db,"nfcDeviceEvents"));adminActivity=snap.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>(activityDate(b.createdAt)?.valueOf()||0)-(activityDate(a.createdAt)?.valueOf()||0));renderActivityLog()}catch(e){console.error("Activity log load failed",e);adminActivity=[];renderActivityLog()}}
$("refreshActivityLog")?.addEventListener("click",loadActivityLog);$("activitySearch")?.addEventListener("input",renderActivityLog);$("activityTypeFilter")?.addEventListener("change",renderActivityLog);
(function initJmxAdminTooltips(){
  ...
  function helpFor(el){
    ...
    if(el.matches?.("[data-delete-nfc-batch],[data-client-delete-batch]"))return "Elimina permanentemente SOLO este lote y sus Device IDs NFC. Conserva la tarjeta/perfil principal del cliente, su Card ID, URL, QR, Google Wallet y los demás lotes.";
  ...
  function ensure(el){
    if(!tip){
      ...
      tip.className="jmx-admin-tooltip";
      tip.setAttribute("role","tooltip");
      tip.id="jmxAdminTooltip";
      // Modern browsers: a manual popover is placed in the browser Top Layer.
      // This is essential because a normal body child can NEVER paint above a showModal() dialog.
      if(typeof tip.showPopover==="function")tip.setAttribute("popover","manual");
      document.body.appendChild(tip);
    }
    // Fallback for browsers without Popover API: keep the tooltip inside the open modal dialog
    // that owns the hovered/touched control so its z-index is evaluated inside that top layer.
  ...
  function place(el){const t=ensure(el),r=el.getBoundingClientRect(),tr=t.getBoundingClientRect(),gap=9;let left=r.left+(r.width-tr.width)/2;left=Math.max(8,Math.min(left,innerWidth-tr.width-8));let top=r.top-tr.height-gap;if(top<8)top=Math.min(innerHeight-tr.height-8,r.bottom+gap);t.style.left=`${Math.round(left)}px`;t.style.top=`${Math.round(top)}px`}
  function show(el,force=false){if(!force&&!finePointer())return;const text=helpFor(el);if(!text)return;current=el;clearTimeout(timer);timer=setTimeout(()=>{if(current!==el)return;const t=ensure(el);t.textContent=text;try{if(typeof t.showPopover==="function"&&!t.matches(":popover-open"))t.showPopover()}catch{}t.classList.add("is-visible");requestAnimationFrame(()=>place(el));el.setAttribute("aria-describedby","jmxAdminTooltip")},300)}
  function hide(el){clearTimeout(timer);if(el&&el.getAttribute?.("aria-describedby")==="jmxAdminTooltip")el.removeAttribute("aria-describedby");current=null;if(tip){tip.classList.remove("is-visible");try{if(typeof tip.hidePopover==="function"&&tip.matches(":popover-open"))tip.hidePopover()}catch{}}}
  ...
  let touchTimer=null,touchTarget=null,suppressNextClick=false;
  document.addEventListener("pointerdown",e=>{if(e.pointerType!=="touch")return;const el=targetFrom(e.target);if(!el)return;touchTarget=el;clearTimeout(touchTimer);touchTimer=setTimeout(()=>{if(touchTarget!==el)return;suppressNextClick=true;show(el,true);navigator.vibrate?.(20)},550)},{passive:true});
  document.addEventListener("pointerup",e=>{if(e.pointerType!=="touch")return;clearTimeout(touchTimer);touchTarget=null;if(suppressNextClick){setTimeout(()=>hide(current),2200)}});
  document.addEventListener("pointercancel",()=>{clearTimeout(touchTimer);touchTarget=null;hide(current)});
