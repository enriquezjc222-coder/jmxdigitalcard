...
async function loadPremiumOwnerStats(){
  ...
  const advanced=String(currentCardPlan).toLowerCase()==="business"&&featureEnabledForPlan("advancedAnalytics");document.querySelectorAll("[data-advanced-analytics]").forEach(el=>el.hidden=!advanced);
...
async function loadAiScannerStatus(){const sec=$id("aiScannerSection");if(!sec)return;const allowed=currentRole!=="none"&&featureEnabledForPlan("aiScanner");sec.hidden=!allowed;if(!allowed)return;const badge=$id("aiScannerAvailability"),usage=$id("aiScannerUsageText");try{const r=(await aiScannerStatusCall({cardId:CARD_ID})).data;const ready=r.externalServicesAllowed===true&&!r.limitReached;if(badge){badge.textContent=ready?"Ready":"Temporarily unavailable";badge.className="scanner-badge "+(ready?"ready":"blocked");}if(usage){const lim=r.limit?.mode==="number"?` / ${r.limit.count}`:"";usage.textContent=`${r.scansThisMonth||0}${lim} scans this month • ${Number(r.allTime?.totalScans||0)} all time • ${r.plan}`;}const saveLead=$id("scannerSaveLead");if(saveLead)saveLead.hidden=!featureEnabledForPlan("leads");document.querySelectorAll(".scanner-capture-button input").forEach(i=>i.disabled=!ready);if(!ready)scannerError(r.limitReached?"Monthly AI Card Scanner limit reached.":"AI Card Scanner is temporarily unavailable. JMX administration may have the external-services Kill Switch turned off.");}catch(e){console.warn("Scanner status unavailable",e);if(badge){badge.textContent="Integration required";badge.className="scanner-badge blocked";}scannerError("AI Card Scanner is temporarily unavailable.");}}
...
async function loadBusinessLeads(){
  ...
  const show=currentRole!=="none"&&String(currentCardPlan).toLowerCase()==="business"&&featureEnabledForPlan("leads");
