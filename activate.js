...
import { getAuth, GoogleAuthProvider, createUserWithEmailAndPassword, signInWithEmailAndPassword, signInWithPopup } from "https://www.gstatic.com/firebasejs/12.17.1/firebase-auth.js";
...
function activationCode() { return $("activationCode").value.trim().toUpperCase(); }
function validateCode() {
  const code = activationCode();
  if (card.requiresActivationCode !== false && !code) {
    msg("Enter the activation code included with your NFC card.", "error");
    return false;
  }
  return true;
}
function setBusy(busy) {
  $("googleActivateButton").disabled = busy;
...
async function claimCard(currentUser) {
  const code = activationCode();
  ...
  batch.set(doc(db, "cardClaims", CARD_ID), { ownerUid: currentUser.uid, ownerEmail: currentUser.email || "", activationCode: code, createdAt: serverTimestamp() });
  batch.set(doc(db, "cardOwners", CARD_ID), { ownerUid: currentUser.uid, ownerEmail: currentUser.email || "", activatedAt: serverTimestamp() });
...
async function activateLegacy() {
  ...
  if (!validateCode()) return;
  setBusy(true); msg("Activating your card…", "working");
  try {
...
$("activateButton").onclick = activateLegacy;
boot();
